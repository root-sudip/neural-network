...
class mlp:
	...
	def load_data(self,path_name,samples=None):
		with open("train_label.csv","r") as fl:
			self.all_array_list = []
			...
			for row in csv_reader:
				...
				self.img_array = np.asarray(Image.open(path_name+'/'+row[0]+'/'+row[1]).resize((32,32), Image.ANTIALIAS))
				self.all_array_list.append(self.img_array)
				self.Y_train[j] = row[2]
			...
			# 				# else: 
			# 				# 	break
			# 		fl.write('sub dir => %s Number og Image => %d label => %d \n' % (subDir,num_of_image,i))
